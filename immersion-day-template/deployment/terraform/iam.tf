...
# Grant application SAs the required permissions in search of the Central Hub Project
resource "google_project_iam_member" "app_sa_central_roles" {
  for_each = {
    for pair in setproduct(keys(local.deploy_project_ids), var.app_sa_roles) :
    join(",", pair) => {
      env  = pair[0]
  ...
  project = var.central_hub_project_id
  role    = each.value.role
